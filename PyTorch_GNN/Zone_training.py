...
# Load data
def load_data(amazon_path, cluster):
    print("Loading data...")
    with open(amazon_path + 'travel_times.json') as f:
        times = json.load(f)
    df = pd.read_parquet(f"zone_dfs/cluster_{cluster}.parquet")
...
def generate_dataset(df, grouped, times):
    ...
    for route_id, route in tqdm(grouped, desc="Generating graphs"):
        ...
        for i, stop in enumerate(route.stop_id):
            ...
            d = {k: v for k, v in d.items() if k in route["stop_id"].values and k != stop}
        ...
        start = route.sort_values(by="order").iloc[0]["stop_id"]
...
# Train
def train(encoder, decoder, baseline_encoder, baseline_decoder, optimizer, train_loader, val_loader, epochs, stops_idx, times, cluster, actual_avg_val, device="cpu", print_every=1):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            rollout_rewards_tensor = -rollout_lengths_tensor

            loss, reward = reinforce_loss(tour_lengths_tensor, log_probs_tensor, baseline=rollout_rewards_tensor)
            ...
            if (batch_idx + 1) % print_every == 0:
                ...
                print(f"\033[92mEpoch {epoch+1}/{epochs}, Batch {batch_idx+1}/{len(train_loader)}, "
                    ...
                    #   f"Avg rewards: {np.mean(rewards):.4f}, "
        ...
        print(f"\033[94mCluster {cluster}: Epoch {epoch+1}/{epochs} completed, Avg Loss: {avg_epoch_loss:.4f}, "
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Please, provide the cluster number")
        sys.exit()

    cluster = sys.argv[1]

    set_config(16)
    df, grouped, times = load_data(amazon_path, cluster)
