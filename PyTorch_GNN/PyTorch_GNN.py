...
def generate_dataset(df, grouped, times):
    ...
    if os.path.exists("graph_lists/graph_list_35nf_1ef.pt"):
    ...
    for route_id, route in tqdm(grouped, desc="Generating graphs"):
        ...
        for i, stop in enumerate(route.stop_id):
            ...
            d = {k: v for k, v in d.items() if k != stop}
...
# Train
def train(encoder, decoder, baseline_encoder, baseline_decoder, optimizer, train_loader, val_loader, epochs, stops_idx, times, device="cpu", print_every=5):
